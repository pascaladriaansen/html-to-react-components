...
HTMLtoJSX.prototype = {
  ...
  _beginVisitElement: function (node) {
    ...
    if('jsx-before' in node.attributes) {
      this.output += '\n' + JSON.stringify(node.attributes['jsx-before']._nodeValue) + '\n';
    }
  ...
  _endVisitElement: function (node) {
    ...
    if('jsx-after' in node.attributes) {
      this.output += '\n' + JSON.stringify(node.attributes['jsx-after']._nodeValue) + '\n';
    }
  ...
  _getElementAttribute: function (node, attribute) {
    switch (attribute.name) {
      ...
      default:
        ...
        // Change jsx-events to just the event name
        if(attribute.name.includes('jsx-on')) {
          var event = attribute.name.replace('jsx-on', '');
          result = 'on' + event.charAt(0).toUpperCase() + event.slice(1);
          event = true;
        }
